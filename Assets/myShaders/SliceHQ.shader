Shader "Custom/SliceHQ"
{
    Properties
    {
        ...
        _GlossMapScale("Smoothness", Range(0.0, 1.0)) = 0.5
        ...
        [Gamma] _Metallic("Metallic Strength", Range(0.0, 2.0)) = 1.0
        ...
        _DetailNormalMapScale("Detail Normal Strength", Range(-2.0, 2.0)) = 1.0
        
        [Enum(UV0,0,UV1,1)] _UVSec("UV Set for secondary textures", Float) = 0.0
        ...
        SubShader
        {
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                // Standard variables
                o.Albedo = c;
                o.Smoothness = _GlossMapScale / 2; //idk why but it is twice as strong as the standard shader so we divide by 2
                o.Alpha = c.a;
                ...
                // metallic map
                float metallicTex = tex2D(_MetallicGlossMap, (IN.uv_MetallicGlossMap + acidEffectOffset) * _MapScale + heightOffset);
                if (step(0.0, metallicTex)) _Metallic = 2.0; // metallic is set to 1.0 if the standard shader uses a metallic albedo map, regardless on the strength in the inspector
                //_Metallic = step(0.0, metallicTex);
                o.Metallic = metallicTex * _Metallic;
                // normal map
                float3 normal_Standard = UnpackScaleNormal(tex2D(_BumpMap, (IN.uv_BumpMap + acidEffectOffset) * _MapScale + heightOffset), _BumpScale * 2); // normal maps are half as strong as standard shader for some reason
                float3 normal_Detail = UnpackScaleNormal(tex2D(_DetailNormalMap, (IN.uv2_DetailNormalMap + acidEffectOffset) * _DetailScale + heightOffset), _DetailNormalMapScale * 2); // normal maps are half as strong as standard shader for some reason

                o.Normal = (normal_Standard + normal_Detail) / 2.0;
                // occlusion map
